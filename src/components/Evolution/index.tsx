...
const Evolution: React.FC = () => {
  ...
  //test grej
   const handlePokemonClick = (selectedPokemon: Pokemon) => {
    ...
    // Scroll to rightcontent
    const rightContentElement = document.getElementById('rightcontent');
    if (rightContentElement) {
      rightContentElement.scrollIntoView({ behavior: 'smooth' });
    }
  };
...
return (
    <>
      <div className={styles.container}>
        <div className={styles.leftcontent}>
          {loading ? (
            ...
            pokeData.map((item, index) => (
              <div className={styles.card} key={`pokemon-key-${item.name}-${item.id}-${index}`} onClick={() => handlePokemonClick(item)}>
                <h2>{item.id}</h2>
        ...
        <div id="rightcontent" className={styles.rightcontainer}>
          <img className={styles["rightcontainer__image"]} src={pokemonlogo}/>
